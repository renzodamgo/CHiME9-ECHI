...
@register_enhancement("joint_ha_uni")
class JointHaUni:
    """Enhancement using Universal GridNet model (ha-joint-uni experiment)"""
    ...
    def __init__(
        ...
        logging.info("UNIVERSAL GRIDNET ENHANCEMENT INITIALIZED")
    ...
    def process_session(
        ...
        """
        Universal GridNet enhancement with single enrollment (Rainbow).
        - Uses Universal GridNet model for speaker-agnostic processing
        ...
        # --- START OF FIX ---
        # The Universal GridNet model requires the speaker input to have the shape [B, K, T, F, 2].
        ...
        for start in tqdm(range(0, duration, self.stride_samples)):
            ...
            logging.info("UNIVERSAL GRIDNET FORWARD PASS:")
            self.model.eval()

            # autocast to bfloat16 for Universal GridNet
            with torch.inference_mode(), autocast("cuda", dtype=torch.bfloat16):
                den_c = self.model(mix_tf, spkid_input, spkid_lens)
            
            logging.info(f"Universal GridNet output shape: {den_c.shape}")  # Should be [1, 1, T, F]
